...
import { BentoItem } from "@/types"; // Import BentoItem interface
                ...
                {/* Actions Overlay - visible on hover or if selected */}
                 {(localHover || isSelected) && (
                    <div className="absolute top-1.5 right-1.5 flex gap-1 z-10 bg-white/80 backdrop-blur-sm rounded-full px-1 py-0.5">
                        <ButtonComponent variant="ghost" size="icon" className={cn("text-gray-500 hover:text-yellow-500 hover:bg-yellow-100 w-6 h-6", item.starred && "text-yellow-400 fill-yellow-400")} onClick={(e) => { e.stopPropagation(); onStar(item.id); }}>
                            <Star className="w-3.5 h-3.5" />
                        </ButtonComponent>
                        <ButtonComponent variant="ghost" size="icon" className="text-gray-500 hover:text-blue-500 hover:bg-blue-100 w-6 h-6" onClick={(e) => { e.stopPropagation(); onShare(item.id); }}>
                            <Share2 className="w-3.5 h-3.5" />
                        </ButtonComponent>
                        <ButtonComponent variant="ghost" size="icon" className="text-gray-500 hover:text-red-500 hover:bg-red-100 w-6 h-6" onClick={(e) => { e.stopPropagation(); onDelete(item.id); }}>
                            <Trash2 className="w-3.5 h-3.5" />
                        </ButtonComponent>
                    </div>
                )}
                {/* Selection Indicator */}
                {isSelected && (<div className="absolute inset-0 border-2 border-black rounded-xl pointer-events-none"></div>
