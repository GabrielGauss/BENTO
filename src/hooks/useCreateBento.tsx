...
// Define FABItem locally if not available
...
const useCreateBento = (
  ...
  const addBentoItem = () => {
    ...
    const type = newBentoType.type;
    let url = newBentoUrl || undefined; // Use url state
    const starred = false;
    // Declare variables for use in switch cases
    let colors: string[];
    let videoId = '';
    let linkText = '';
    ...
    switch (type) {
        ...
        content = (
          <>
            <img
              ...
              onError={(e) =>
                (e.currentTarget.src =
                  "https://placehold.co/200x300/CCCCCC/999999?text=Invalid+URL")
              }
            />
      ...
      case "youtube":
        ...
        if (!videoId) {
          console.warn(
            "Could not extract YouTube video ID. Please use a valid YouTube video URL."
          );
          return;
        }
      ...
      case "audio":
        ...
        break;
      case "link":
        linkText = newBentoUrl; // Default to URL
        try {
          linkText = new URL(newBentoUrl).hostname; // Try to get hostname
        } catch {
          /* Ignore invalid URL for text generation */
        }
        content = (
          <a
            href={newBentoUrl}
